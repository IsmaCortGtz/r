...
function getECLevelOffset(errorCorrectionLevel: ECLevel.ECLevel): number | undefined {
	switch (errorCorrectionLevel) {
		case ECLevel.L: return 0
		case ECLevel.M: return 1
		case ECLevel.Q: return 2
		case ECLevel.H: return 3
		default: return undefined
	}
}

export function getBlocksCount(version: number, errorCorrectionLevel: ECLevel.ECLevel): number | undefined {
	const offset = getECLevelOffset(errorCorrectionLevel)
	return offset !== undefined ? EC_BLOCKS_TABLE[(version - 1) * 4 + offset] : undefined
}

export function getTotalCodewordsCount(version: number, errorCorrectionLevel: ECLevel.ECLevel): number | undefined {
	const offset = getECLevelOffset(errorCorrectionLevel)
	return offset !== undefined ? EC_CODEWORDS_TABLE[(version - 1) * 4 + offset] : undefined
}
